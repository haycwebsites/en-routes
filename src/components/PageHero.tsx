...
interface PageHeroProps {
  ...
  /** Optional hero background image URL (e.g. from innerPageHeroImages) */
  backgroundImage?: string;
}

export function PageHero({ title, titleConfigPath, breadcrumbs = [], backgroundImage }: PageHeroProps) {
  ...
  return (
    <section className="relative py-24 md:py-32 bg-gray-900 overflow-hidden">
      {backgroundImage && (
        <>
          <img src={backgroundImage} alt="" className="absolute inset-0 w-full h-full object-cover object-center" aria-hidden />
