    ...
    <>
      ...
      <section className="section-padding">
        <div className="container-custom max-w-3xl">
          ...
          <img src={img(item.image ?? FALLBACK_GREECE_IMAGE)} alt="" className="aspect-video w-full rounded-lg object-cover object-center bg-gray-700 mb-8" />
          <div className="prose prose-invert max-w-none">
            <p className="text-white/70 mb-4" {...cp('blogSinglePageConfig.item.body1')}>{t(item.body1)}</p>
            <p className="text-white/70 mb-4" {...cp('blogSinglePageConfig.item.body2')}>{t(item.body2)}</p>
            <p className="text-white/70 mb-8" {...cp('blogSinglePageConfig.item.body3')}>{t(item.body3)}</p>
          </div>
